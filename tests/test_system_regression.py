...
class SystemRegressionTests(unittest.TestCase):
    ...
    def test_02_member_auth_approval_profile_and_search_logs(self):
        ...
        self.assertTrue(audit_detail["audit_key"])
        ...
        copy_contexts = app["load_member_search_copy_contexts"](audit_ids)
        self.assertEqual(copy_contexts[audit_ids[0]]["audit_key"], audit_detail["audit_key"])
        self.assertEqual(copy_contexts[audit_ids[0]]["query_text"], "0402 10K 1% 1/16W")
    ...
    def test_02a0_search_result_copy_audit_is_signed_invisible_and_traceable(self):
        app = self.app
        original_copy_config = app["get_member_search_copy_api_config"]
        secret = "copy-audit-regression-secret"
        app["get_member_search_copy_api_config"] = lambda: (
            "https://example.test/api/member-search-copy",
            secret,
            True,
        )
        try:
            ticket = app["build_member_search_copy_ticket"](
                321,
                9,
                "matched",
                2,
                ...
                search_context={
                    "audit_key": "audit-321-stable",
                    "username_snapshot": "sales1",
                    "display_name_snapshot": "销售一",
                    "customer_name_snapshot": "客户A有限公司",
                    "query_text": "0603 10K 1% 1/10W",
                    "created_at": "2026-08-26 16:20:00",
                },
            )
            encoded_payload, signature = ticket.split(".", 1)
            expected_signature = app["hmac"].new(
                secret.encode("utf-8"),
                encoded_payload.encode("ascii"),
                app["hashlib"].sha256,
            ).hexdigest()
            self.assertEqual(signature, expected_signature)
            payload_padding = "=" * ((4 - len(encoded_payload) % 4) % 4)
            payload = json.loads(
                base64.urlsafe_b64decode(encoded_payload + payload_padding).decode("utf-8")
            )
            self.assertEqual(payload["log"], 321)
            self.assertEqual(payload["member"], 9)
            self.assertEqual(payload["brand"], "FOJAN(富捷)")
            self.assertEqual(payload["model"], "FRC0603F1002TS")
            self.assertEqual(payload["audit"], "audit-321-stable")
            self.assertEqual(payload["username"], "sales1")
            self.assertEqual(payload["display"], "销售一")
            self.assertEqual(payload["customer"], "客户A有限公司")
            self.assertEqual(payload["query"], "0603 10K 1% 1/10W")
            self.assertEqual(payload["searched"], "2026-08-26 16:20:00")

            fallback_ticket = app["build_member_search_copy_ticket"](
                0,
                9,
                "matched",
                2,
                ...
                issued_at_ts=1_800_000_000,
                search_context={
                    "audit_key": "independent-copy-audit",
                    "username_snapshot": "sales1",
                    "query_text": "0603 10K 1% 1/10W",
                },
            )
            fallback_encoded = fallback_ticket.split(".", 1)[0]
            fallback_padding = "=" * ((4 - len(fallback_encoded) % 4) % 4)
            fallback_payload = json.loads(
                base64.urlsafe_b64decode(fallback_encoded + fallback_padding).decode("utf-8")
            )
            self.assertEqual(fallback_payload["log"], 0)
            self.assertEqual(fallback_payload["audit"], "independent-copy-audit")

            result_html = app["render_clickable_result_table"](
                pd.DataFrame([{"品牌": "FOJAN(富捷)", "型号": "FRC0603F1002TS"}]),
                show_official_status=False,
                copy_audit={"search_log_id": 321, "member_id": 9, "result_group": "matched"},
                copy_bridge_channel="bridge-regression",
            )
        finally:
            app["get_member_search_copy_api_config"] = original_copy_config

        self.assertIn('data-copy-audit-ticket="', result_html)
        self.assertIn('data-copy-audit-field="品牌"', result_html)
        self.assertIn('data-copy-audit-field="型号"', result_html)
        self.assertIn("document.addEventListener('copy'", result_html)
        self.assertIn("source: 'fruition-search-copy'", result_html)
        self.assertIn('var copyBridgeChannel = "bridge-regression"', result_html)
        self.assertNotIn("clipboard.read", result_html.lower())
        copy_listener = result_html.split("document.addEventListener('copy'", 1)[1].split(
            "function toNumber", 1
        )[0]
        self.assertNotIn("preventDefault", copy_listener)

        copy_events = [
            {
                "search_log_id": 999,
                "search_audit_key": "audit-321-stable",
                "username_snapshot": "sales1",
                "display_name_snapshot": "销售一",
                "customer_name_snapshot": "客户A有限公司",
                "query_text": "0603 10K 1% 1/10W",
                "search_created_at": "2026-08-26 16:20:00",
                "result_group": "matched",
                "result_rank": 2,
                "brand": "FOJAN(富捷)",
                "model": "FRC0603F1002TS",
                "received_at": "2026-08-26T08:30:00.000Z",
            }
        ]
        detail_df = app["member_search_detail_dataframe"](
            [
                {
                    "id": 321,
                    "audit_key": "audit-321-stable",
        ...
        self.assertEqual(int(detail_df.iloc[0]["复制次数"]), 1)
        self.assertEqual(detail_df.iloc[0]["已复制品牌/型号"], "FOJAN(富捷) / FRC0603F1002TS")
        self.assertEqual(
            list(detail_df.columns)[6:9],
            ["复制次数", "已复制品牌/型号", "最近复制时间"],
        )
        audit_df = app["member_search_audit_results_dataframe"](
            [
                {
                    "result_group": "matched",
                    "result_rank": 2,
        ...
        self.assertEqual(audit_df.iloc[0]["是否复制"], "是")
        self.assertEqual(int(audit_df.iloc[0]["复制次数"]), 1)
        copy_df = app["member_search_copy_events_dataframe"](copy_events)
        self.assertEqual(copy_df.iloc[0]["搜索记录"], "#999")
        self.assertEqual(copy_df.iloc[0]["账号"], "sales1")
        self.assertEqual(copy_df.iloc[0]["客户"], "客户A有限公司")
        self.assertEqual(copy_df.iloc[0]["搜索内容"], "0603 10K 1% 1/10W")

        duplicate_events = [
            {
                **copy_events[0],
                "event_id": f"duplicate-{index}",
                "client_copied_at": f"2026-08-26T08:30:00.{index * 200:03d}Z",
                "received_at": f"2026-08-26T08:30:00.{index * 20:03d}Z",
            }
        ...
        duplicate_events.append(
            {
                **copy_events[0],
                "event_id": "intentional-later-copy",
                "client_copied_at": "2026-08-26T08:30:03.000Z",
                "received_at": "2026-08-26T08:30:03.000Z",
            }
        )
        deduplicated = app["deduplicate_member_search_copy_events"](duplicate_events)
        self.assertEqual(len(deduplicated), 2)
        self.assertEqual(deduplicated[0]["event_id"], "intentional-later-copy")

        merged_details = app["merge_member_search_details_with_copy_events"]([], duplicate_events)
        self.assertEqual(len(merged_details), 1)
        self.assertEqual(merged_details[0]["id"], 999)
        self.assertEqual(merged_details[0]["query_text"], "0603 10K 1% 1/10W")
        self.assertEqual(merged_details[0]["source"], "远程复制审计")
        merged_detail_df = app["member_search_detail_dataframe"](
            merged_details,
            copy_events=duplicate_events,
        )
        self.assertEqual(int(merged_detail_df.iloc[0]["复制次数"]), 2)
        self.assertEqual(
            merged_detail_df.iloc[0]["已复制品牌/型号"],
            "FOJAN(富捷) / FRC0603F1002TS",
        )

        with open(
            os.path.join(self.base_dir, "cloudflare-pages-proxy", "dist", "_worker.js"),
            "r",
            encoding="utf-8",
        ) as worker_handle:
            worker_source = worker_handle.read()
        self.assertIn('dispatchPath === "/api/member-search-copy"', worker_source)
        self.assertIn("verifyMemberSearchCopyTicket", worker_source)
        self.assertIn("search_audit_key TEXT NOT NULL DEFAULT ''", worker_source)
        self.assertIn('url.searchParams.get("search_audit_key")', worker_source)
        self.assertIn("username_snapshot, display_name_snapshot, customer_name_snapshot", worker_source)
        self.assertIn("(searchLogId <= 0 && !searchAuditKey)", worker_source)
        self.assertIn('void fetch("/api/member-search-copy"', worker_source)
        self.assertIn("keepalive: true", worker_source)
        self.assertIn('payload_encoding TEXT NOT NULL DEFAULT \'identity\'', worker_source)
        self.assertIn('new DecompressionStream("gzip")', worker_source)
        self.assertIn("uncompressed_size INTEGER NOT NULL DEFAULT 0", worker_source)
    ...
    def test_03ab_nonstandard_fojan_resistance_explains_no_match(self):
        app = self.app
        query = "贴片电阻 17KΩ ±1% 0603 1/10W 0603 无卤"
        mode, spec = app["detect_query_mode_and_spec"](pd.DataFrame(), query)
        spec = app["merge_query_text_hints_into_spec"](spec, query)

        self.assertEqual(mode, "厚膜电阻")
        self.assertEqual(app["build_fojan_resistor_model_from_spec"](spec), "")
        self.assertEqual(
            app["fojan_neighboring_standard_resistances"](17_000.0, "1"),
            (16_900.0, 17_400.0),
        )
        notice = app["build_fojan_nonstandard_resistance_notice"](spec)
        self.assertIn("E24+E96", notice)
        self.assertIn("17KΩ 不在标准阻值表内", notice)
        self.assertIn("“无卤”不是本次无结果的原因", notice)
        self.assertIn("16.9KΩ、17.4KΩ", notice)
        self.assertIn("不会被当作当前阻值的精确替代", notice)

        for valid_query, expected_model in (
            ("贴片电阻 240Ω ±5% 1206 1/4W 无卤", "FRC1206J241 TS"),
            ("2MΩ ±5% 0402 1/16W 无卤", "FRC0402J205 TS"),
        ):
            valid_mode, valid_spec = app["detect_query_mode_and_spec"](pd.DataFrame(), valid_query)
            valid_spec = app["merge_query_text_hints_into_spec"](valid_spec, valid_query)
            self.assertIn(valid_mode, {"厚膜电阻", "贴片电阻"})
            self.assertEqual(
                app["build_fojan_resistor_model_from_spec"](valid_spec),
                expected_model,
                valid_query,
            )
            self.assertEqual(app["build_fojan_nonstandard_resistance_notice"](valid_spec), "")

    def test_03ac_fojan_frl_part_lookup_matches_equivalent_spec(self):
        app = self.app
        part_query = "FRL2512FR039TS"
        direct_query = "2512 0.039R 1% 1W"

        part_result = app["resolve_search_query_dataframe_and_spec"](
            part_query,
            get_full_search_df=lambda: pd.DataFrame(),
        )
        direct_result = app["resolve_search_query_dataframe_and_spec"](
            direct_query,
            get_full_search_df=lambda: pd.DataFrame(),
        )
        part_matches = app["run_query_match"](
            part_result["query_df"],
            part_result["mode"],
            part_result["spec"],
        )
        direct_matches = app["run_query_match"](
            direct_result["query_df"],
            direct_result["mode"],
            direct_result["spec"],
        )

        self.assertEqual(part_result["mode"], "料号")
        self.assertTrue(part_result["spec"].get("_part_lookup_source_metadata_relaxed"))
        self.assertTrue(part_result["spec"].get("_fojan_frl_part_lookup_relaxed"))
        self.assertEqual(part_result["spec"]["_source_component_type"], "厚膜电阻")
        self.assertEqual(part_result["spec"]["_source_material"], "厚膜")
        self.assertEqual(part_result["spec"]["_source_voltage"], "200")
        self.assertEqual(part_result["spec"]["_source_special_use"], "低阻值")
        self.assertEqual(part_result["spec"]["器件类型"], "贴片电阻")
        self.assertEqual(part_result["spec"]["材质（介质）"], "")
        self.assertEqual(part_result["spec"]["耐压（V）"], "")
        self.assertEqual(part_result["spec"]["特殊用途"], "")

        part_models = set(part_matches["型号"].astype(str).map(app["clean_model"]))
        direct_models = set(direct_matches["型号"].astype(str).map(app["clean_model"]))
        self.assertEqual(part_models, direct_models)
        self.assertGreaterEqual(len(part_models), 8)
        self.assertIn("FRL2512FR039TS", part_models)
        self.assertIn("FBF25FP-R039", part_models)
        self.assertIn("25121WF390MT4E", part_models)
        self.assertIn("RL2512FK-070R039L", part_models)

        part_info = app["build_part_info_df"](
            part_result["query_df"],
            part_result["spec"],
            part_query,
        )
        self.assertEqual(part_info.iloc[0]["型号"], "FRL2512FR039TS")
        self.assertIn("厚膜电阻", part_info.iloc[0]["器件类别"])
        other_brand_matches = app["exclude_brand_model_pairs"](
            part_matches,
            app["extract_brand_model_pairs"](part_info),
        )
        self.assertEqual(len(other_brand_matches), len(part_matches) - 1)
        self.assertNotIn(
            "FRL2512FR039TS",
            set(other_brand_matches["型号"].astype(str).map(app["clean_model"])),
        )

    def test_03ad_psa_fcf_part_lookup_drops_false_low_ohm_series_filter(self):
        app = self.app
        part_query = "FCF02FV-1501"
        part_result = app["resolve_search_query_dataframe_and_spec"](
            part_query,
            get_full_search_df=lambda: pd.DataFrame(),
        )
        ...
        self.assertEqual(part_result["mode"], "料号")
        self.assertTrue(part_result["spec"].get("_part_lookup_source_metadata_relaxed"))
        self.assertEqual(part_result["spec"]["_source_special_use"], "电流检测/低阻值")
        self.assertEqual(part_result["spec"]["_source_required_special_use"], "")
        self.assertEqual(part_result["spec"]["特殊用途"], "")
        self.assertAlmostEqual(float(part_result["spec"]["_resistance_ohm"]), 1500.0)

        part_models = set(part_matches["型号"].astype(str).map(app["clean_model"]))
        self.assertGreaterEqual(len(part_models), 25)
        self.assertGreaterEqual(part_matches["品牌"].nunique(), 12)
        self.assertIn("FRC0402F1501TS", part_models)
        self.assertIn("WR04X1501FTL", part_models)
        self.assertIn("RC0402FR-071K5L", part_models)
        self.assertNotIn("FCF02FV-1501", part_models)

    def test_03ae_special_resistor_part_lookup_keeps_credible_series_requirements(self):
        app = self.app
        part_query = "NQ02WGF1501TCE"
        part_result = app["resolve_search_query_dataframe_and_spec"](
            part_query,
            get_full_search_df=lambda: pd.DataFrame(),
        )
        ...
        self.assertTrue(part_result["spec"].get("_part_lookup_source_metadata_relaxed"))
        self.assertEqual(part_result["spec"]["特殊用途"], "车规/抗硫化")
        self.assertEqual(
            part_result["spec"]["_source_required_special_use"],
            "车规/抗硫化",
        )

        matches_by_model = {
            app["clean_model"](row["型号"]): row
            for _, row in part_matches.iterrows()
        }
        self.assertIn("RMS04FT1501", matches_by_model)
        self.assertEqual(matches_by_model["RMS04FT1501"]["推荐等级"], "完全匹配")
        self.assertNotIn("FRC0402F1501TS", matches_by_model)
    ...
    def test_05g_admin_cost_customer_choices_are_searchable_and_exclude_disabled_customers(self):
        ...
        original_cost_path = app["COST_PRICE_DB_PATH"]
        try:
            app["COST_PRICE_DB_PATH"] = os.path.join(self.temp_dir, "admin-cost-customer-selector.sqlite")
            app["clear_cost_price_lookup_cache"]()
            for name, code, group, active in [
                ("A集团深圳有限公司", "F0001", "A集团", True),
                ("B集团上海有限公司", "B0001", "B集团", True),
                ("已停用客户有限公司", "X0001", "停用集团", False),
            ]:
                ok, message, _ = app["save_sales_customer"](
                    name,
                    code,
                ...
                self.assertTrue(ok, message)
            for customer_name in ["历史报价客户有限公司", "已停用客户有限公司"]:
                ok, message, _ = app["save_manual_cost_price_item"](
                    "FOJAN",
                    f"SELECTOR-{app['normalize_cost_customer_key'](customer_name)}",
                    "1.00",
            ...
            choices = app["build_admin_cost_customer_choices"]()
            by_name = {row["customer_name"]: row for row in choices}
            self.assertIn("A集团深圳有限公司", by_name)
            self.assertIn("B集团上海有限公司", by_name)
            self.assertIn("历史报价客户有限公司", by_name)
            self.assertNotIn("已停用客户有限公司", by_name)
            self.assertIn("F0001", by_name["A集团深圳有限公司"]["label"])
            self.assertIn("A集团", by_name["A集团深圳有限公司"]["label"])
            self.assertIn("历史专属价客户", by_name["历史报价客户有限公司"]["label"])
        finally:
            app["COST_PRICE_DB_PATH"] = original_cost_path
            app["clear_cost_price_lookup_cache"]()
    ...
    def test_06_bom_full_read_export_and_display_columns(self):
        ...
        customer_bom_mapping = app["guess_bom_column_mapping"](
            pd.DataFrame(
                {
                    "Category": ["RES", "", ""],
                    "昶音料号": ["M3216019000LB", "M3216019001LB", "M3216019002LB"],
                    "中文名称": ["贴片电阻", "贴片电阻", "贴片电阻"],
                    "物料名称": [
                        "RES,0R,+/-5%,0201,1/20W",
                        "RES,10K,+/-1%,0402,1/16W",
                        "RES,1K,+/-1%,0603,1/10W",
                    ],
                    "厂商代码": ["0201WMJ0000TCE", "RC0402FR-0710KL", "RC0603FR-071KL"],
                    "高度mm（max）": [0.3, 0.4, 0.5],
                }
            )
        )
        self.assertEqual(customer_bom_mapping["model"], "厂商代码")
        self.assertEqual(customer_bom_mapping["spec"], "物料名称")
        ...
        try:
            app["find_labeled_current_in_text"] = lambda *args, **kwargs: (
                current_parser_calls.append((args, kwargs)) or ""
            )
            resistor_detail = app["build_component_spec_detail_from_row"](
                {
                    "器件类型": "厚膜电阻",
                    "尺寸（inch）": "0402",
                    "容值": "10",
            ...
            app["find_labeled_current_in_text"] = original_current_parser
        self.assertEqual(current_parser_calls, [])
        self.assertIn("阻值", resistor_detail)
    ...
    def test_07_member_database_remote_snapshot_survives_instance_reset(self):
                ...
                snapshot.update(
                    {
                        ...
                        "uncompressed_size": int(body.get("uncompressed_size") or 0),
        ...
        try:
            ...
            self.assertEqual(snapshot["payload_encoding"], "gzip")
            stored_snapshot_payload = base64.b64decode(snapshot["payload_base64"])
            self.assertLess(len(stored_snapshot_payload), int(snapshot["uncompressed_size"]))
            ...
            remote_payload = base64.b64decode(snapshot["payload_base64"])
            if snapshot.get("payload_encoding") == "gzip":
            ...
            snapshot.update(
                {
                    ...
                    "payload_encoding": "identity",
                    "uncompressed_size": len(stale_session_payload),
    ...
    def test_14_fojan_alloy_resistor_rules_are_source_scoped(self):
        ...
        for query in (
            "0805检流电阻(0.005Ω,1%,1/2W)",
            "0805檢流電阻(0.005Ω,1%,1/2W)",
        ):
            mode, spec = app["detect_query_mode_and_spec"](pd.DataFrame(), query)
            self.assertEqual(mode, "合金电阻", query)
            self.assertEqual(spec["尺寸（inch）"], "0805", query)
            self.assertAlmostEqual(spec["_resistance_ohm"], 0.005, places=9, msg=query)
            self.assertEqual(spec["容值误差"], "1", query)
            self.assertEqual(app["format_power_display"](spec["_power"]), "1/2W", query)
            ...
            self.assertIn("FRM0805FR005TM", fojan_models, query)
    ...
    def test_16_chip_metric_size_is_converted_for_matching_and_both_codes_are_displayed(self):
        app = self.app
        query = "R-CHIP;47 ohm,1%,1/16W,1005"
        mode, spec = app["detect_query_mode_and_spec"](pd.DataFrame(), query)

        self.assertEqual(mode, "贴片电阻")
        self.assertEqual(spec["尺寸（公制）"], "1005")
        self.assertEqual(spec["尺寸（inch）"], "0402")

        spec_display = app["build_spec_info_df"](spec)
        self.assertIn("尺寸（公制）", spec_display.columns)
        self.assertIn("尺寸（英制）", spec_display.columns)
        self.assertNotIn("尺寸（inch）", spec_display.columns)
        self.assertEqual(spec_display.iloc[0]["尺寸（公制）"], "1005")
        self.assertEqual(spec_display.iloc[0]["尺寸（英制）"], "0402")

        rows = app["load_search_dataframe_for_query"](mode, spec, query)
        matches = app["run_query_match"](rows, mode, spec)
        self.assertFalse(matches.empty)
        display = app["select_component_display_columns"](
            matches.head(5),
            spec,
            prefix_columns=["推荐等级", "品牌", "型号"],
        )
        self.assertTrue(display["尺寸（公制）"].eq("1005").all())
        self.assertTrue(display["尺寸（英制）"].eq("0402").all())

        imperial_mode, imperial_spec = app["detect_query_mode_and_spec"](
            pd.DataFrame(),
            "贴片电阻 47Ω 1% 1/16W 0402",
        )
        self.assertIn(imperial_mode, {"贴片电阻", "厚膜电阻"})
        self.assertEqual(imperial_spec["尺寸（公制）"], "1005")
        self.assertEqual(imperial_spec["尺寸（inch）"], "0402")

        mlcc_mode, mlcc_spec = app["detect_query_mode_and_spec"](
            pd.DataFrame(),
            "MLCC 1005 X7R 100nF 16V 10%",
        )
        self.assertEqual(mlcc_mode, "规格")
        self.assertEqual(mlcc_spec["尺寸（公制）"], "1005")
        self.assertEqual(mlcc_spec["尺寸（inch）"], "0402")
        mlcc_display = app["build_spec_info_df"](mlcc_spec)
        self.assertEqual(mlcc_display.iloc[0]["尺寸（公制）"], "1005")
        self.assertEqual(mlcc_display.iloc[0]["尺寸（英制）"], "0402")

    def test_17_component_tables_size_columns_from_visible_content(self):
        app = self.app
        query = "贴片电阻 100KΩ 5% 1/16W 1005"
        _, spec = app["detect_query_mode_and_spec"](pd.DataFrame(), query)
        spec_display = app["build_spec_info_df"](spec)
        config = app["build_component_column_config"](spec_display, spec)

        self.assertTrue(all(isinstance(item.get("width"), int) for item in config.values()))
        self.assertLessEqual(config["尺寸（公制）"]["width"], 110)
        self.assertLessEqual(config["尺寸（英制）"]["width"], 110)
        self.assertLess(config["系列"]["width"], config["系列说明"]["width"])
        self.assertEqual(config["系列"]["width"], 64)

        short_width = app["estimate_result_table_column_width"]("系列", ["FRC"], "系列")
        long_width = app["estimate_result_table_column_width"](
            "系列",
            ["LONG-SERIES-CODE-WITH-MORE-CONTENT"],
            "系列",
        )
        self.assertEqual(short_width, 64)
        self.assertGreater(long_width, short_width)

        models = ["FRC0402J104TS"] * 99 + ["VERY-LONG-MODEL-NUMBER-AT-THE-END-OF-THE-VISIBLE-SAMPLE"]
        table_html = app["render_clickable_result_table"](
            pd.DataFrame({"型号": models}),
            show_official_status=False,
            wrap_iframe=False,
        )
        self.assertIn('<col style="width: 320px;" />', table_html)

    def test_18_repeated_component_prefixes_split_pasted_searches_without_newlines(self):
        app = self.app
        pasted = (
            "R-CHIP;100K ohm,5%,1/16W,1005"
            "R-CHIP;1.5K ohm,5%,1/16W,1005"
            "R-CHIP;3.3K ohm,5%,1/16W,1005"
            "R-CHIP;39K ohm,5%,1/16W,1005"
            "R-CHIP;560 ohm,5%,1/16W,1005"
            "R-CHIP;75 ohm,5%,1/16W,"
        )
        lines = app["split_search_query_input"](pasted)

        self.assertEqual(len(lines), 6)
        self.assertTrue(all(line.startswith("R-CHIP;") for line in lines))
        expected_values = [100000.0, 1500.0, 3300.0, 39000.0, 560.0, 75.0]
        for line, expected_value in zip(lines, expected_values):
            mode, spec = app["detect_query_mode_and_spec"](pd.DataFrame(), line)
            self.assertIn(mode, {"贴片电阻", "厚膜电阻"})
            self.assertEqual(spec["_resistance_ohm"], expected_value)
            self.assertGreaterEqual(spec["_param_count"], 3)
        for line in lines[:-1]:
            _, spec = app["detect_query_mode_and_spec"](pd.DataFrame(), line)
            self.assertEqual(spec["尺寸（公制）"], "1005")
            self.assertEqual(spec["尺寸（inch）"], "0402")

        mixed = "C-CHIP;100nF,10%,16V,1005R-CHIP;1K ohm,1%,1/16W,1005"
        self.assertEqual(
            app["split_search_query_input"](mixed),
            [
                "C-CHIP;100nF,10%,16V,1005",
                "R-CHIP;1K ohm,1%,1/16W,1005",
            ],
        )
        self.assertEqual(
            app["split_search_query_input"]("备注 R-CHIP;1K ohm,1%,1/16W,1005"),
            ["备注 R-CHIP;1K ohm,1%,1/16W,1005"],
        )

        with open(os.path.join(self.base_dir, "component_matcher.py"), "r", encoding="utf-8") as handle:
            source = handle.read()
        self.assertIn("lines = split_search_query_input(query_input)", source)

    def test_19_pdc_fbf_official_model_is_reverse_parsed_and_returns_matches(self):
        app = self.app
        model = "FBF05FTPR100"
        model_key = app["clean_model"](model)

        parsed = app["parse_pdc_fbf_resistor_model"](model)
        self.assertIsNotNone(parsed)
        self.assertEqual(parsed["品牌"], "信昌PDC")
        self.assertEqual(parsed["系列"], "FBF")
        self.assertEqual(parsed["器件类型"], "厚膜电阻")
        self.assertEqual(parsed["尺寸（inch）"], "0805")
        self.assertEqual(parsed["_resistance_ohm"], 0.1)
        self.assertEqual(parsed["容值误差"], "1")
        self.assertEqual(parsed["_power"], "1/2W")
        self.assertEqual(parsed["最高工作电压"], "0.224")
        self.assertEqual(parsed["耐压（V）"], "0.224")
        self.assertEqual(parsed["温度系数"], "±100ppm/℃")
        self.assertIn("无卤", parsed["特殊用途"])
        self.assertEqual(parsed["_model_rule_authority"], "pdc_fbf_official_model")

        source_rows = app["load_component_rows_by_exact_models_from_database"]([model])[model_key]
        self.assertFalse(source_rows.empty)
        self.assertEqual(set(source_rows["型号"].map(app["clean_model"])), {model_key})
        mode, spec = app["detect_query_mode_and_spec"](source_rows, model)
        self.assertEqual(mode, "料号")
        self.assertEqual(spec["尺寸（inch）"], "0805")
        self.assertEqual(spec["_resistance_ohm"], 0.1)
        self.assertEqual(spec["容值误差"], "1")
        self.assertEqual(spec["_power"], "1/2W")
        ...
        self.assertEqual(resolved["resolution_path"], "fast_query")
        self.assertTrue(resolved["spec"].get("_pdc_fbf_part_lookup_relaxed", False))
        part_info = app["build_part_info_df"](source_rows, resolved["spec"], model)
        self.assertEqual(set(part_info["型号"].map(app["clean_model"])), {model_key})
        ...
        self.assertFalse(matches.empty)
        self.assertNotIn(model_key, set(matches["型号"].map(app["clean_model"])))
        self.assertTrue(
            matches["品牌"].astype(str).str.contains("Panasonic", case=False, regex=False).any()
        )

        standard_high_ohm = app["parse_pdc_fbf_resistor_model"]("FBF06FT-3R00N")
        self.assertEqual(standard_high_ohm["尺寸（inch）"], "1206")
        self.assertEqual(standard_high_ohm["_resistance_ohm"], 3.0)
