...
class MemberAuthBridgeSourceTests(unittest.TestCase):
    ...
    def test_formal_shell_persists_active_page_mode_across_browser_refresh(self):
        self.assertIn("active_page_mode = requested_page_mode()", self.matcher)
        self.assertIn('action: "set-page-mode"', self.matcher)
    ...
    def test_formal_shell_persists_active_admin_module_across_browser_refresh(self):
        ...
        self.assertIn('"搜索记录": "search_logs"', self.matcher)
        self.assertIn('adminModule: activeAdminModule', self.matcher)
        self.assertIn('const adminModuleValues = new Set([', self.worker)
        self.assertIn('frameUrl.searchParams.set("admin_module", outerAdminModule);', self.worker)
        self.assertIn('routeUrl.searchParams.delete("admin_module");', self.worker)
        self.assertIn('routeUrl.searchParams.set("admin_module", adminModule);', self.worker)

        admin_start = self.matcher.index("def render_no_match_report_admin_page():")
        admin_end = self.matcher.index("\ndef ", admin_start + 5)
        admin_function = self.matcher[admin_start:admin_end]
        self.assertIn("requested_module_slug", admin_function)
        self.assertIn("ADMIN_BACKEND_MODULE_LABEL_BY_SLUG.get", admin_function)
        self.assertIn("ADMIN_BACKEND_MODULE_SLUG_BY_LABEL.get", admin_function)
        self.assertIn("st.rerun()", admin_function)

    def test_copy_summary_is_integrated_into_search_details(self):
        admin_start = self.matcher.index("def render_member_search_logs_admin_page():")
        admin_end = self.matcher.index("\ndef render_member_approval_admin_page", admin_start)
        admin_function = self.matcher[admin_start:admin_end]
        self.assertNotIn('st.markdown("#### 最近复制的品牌和型号")', admin_function)
        self.assertIn('st.markdown("#### 搜索规格／复制型号明细")', admin_function)
        self.assertNotIn('with st.expander("查看搜索明细"', admin_function)
