...
- Root cause: Streamlit renders the persistence script inside a sandboxed nested component iframe, so it cannot directly write the Cloudflare shell's cookie or localStorage. Sending to `window.top` solved the frame boundary but needed an explicit trusted receiver and message correlation.
- Fix: The Streamlit component now posts only to `https://fruition-component.pages.dev`. The Cloudflare shell creates a cryptographically random per-load channel, passes it into the Streamlit iframe, accepts only messages with that channel, stores the token for one hour, injects it on reopen, and removes one-time `member_token` values from the visible shell URL.
