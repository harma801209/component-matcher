...
## 2026-08-24 - Current-sense wording bypassed FOJAN alloy rules

- Symptom: the complete specification `0805检流电阻(0.005Ω,1%,1/2W)` returned no result and showed the unavailable full-library fallback warning even though FOJAN officially supports an FRM 0805 5mΩ ±1% 0.5W alloy resistor.
- Root cause: the alloy-resistor alias table covered `采样电阻`, `取样电阻`, and `电流检测电阻`, but omitted the common Chinese term `检流电阻`. The parser therefore classified the line as an ordinary chip resistor and never invoked the source-scoped FOJAN FRM generator.
- Fix: recognize simplified `检流电阻` and traditional `檢流電阻` as explicit alloy/current-sense resistor terms. The existing electrical parser and official FRM range checks remain unchanged.
- Regression: both the simplified and traditional forms parse as `合金电阻`, retain 0805/5mΩ/±1%/0.5W, and return `FRM0805FR005TM`. The complete 54-test release safety gate passes with protected runtime database fingerprints unchanged.

## 2026-08-26 - A valid non-standard resistance rejection looked like a database failure

- Symptom: `贴片电阻 17KΩ ±1% 0603 1/10W 0603 无卤` returned no model with only the generic message to check the database. In the same report, `240Ω ±5% 1206 1/4W` and `2MΩ ±5% 0402 1/16W` appeared to be failures even though both already returned FOJAN results.
- Root cause: FOJAN's official FRC datasheet specifies E24+E96 for ±1% values and confirms the series is halogen-free. 17KΩ is not an E24 or E96 value, and the current component library contains no exact 17KΩ row; the zero-result decision was safe, but the generic warning hid that distinction.
- Fix: detect a FOJAN FRC non-standard value only after an exact match has failed, explain the official series rule, state that halogen-free is not the blocker, and display the adjacent standard values 16.9KΩ and 17.4KΩ for engineering confirmation. Do not generate `FRC0603F1702TS` and do not rank either neighboring value as an exact match.
- Regression: the reported 17KΩ input is locked to the explanatory path, while the 240Ω and 2MΩ controls remain `FRC1206J241 TS` and `FRC0402J205 TS`. The complete release gate reports 55 passing tests and no protected runtime-data change.

## 2026-08-26 - FOJAN FRL exact-part lookup over-constrained its alternatives

- Symptom: searching `FRL2512FR039TS` displayed the FOJAN source row but no other-brand result; searching its electrical specification `2512 0.039R 1% 1W` returned 18 matching models.
- Root cause: reverse lookup copied source-row classification metadata (`厚膜电阻`, `厚膜`, `200V`, and `低阻值`) into hard candidate constraints. The direct specification route used the intended electrical core—2512, 39mΩ, ±1%, and 1W—so the two entry points were not equivalent.
- Fix: for a bare, exact FOJAN FRL order number, retain all source metadata separately for the source-information card while searching alternatives as a generic chip resistor on the same size/resistance/tolerance/power core. This narrow rule does not relax explicit compound specifications, automotive/special families, or alloy-resistor routing.
- Regression: exact-part and direct-spec routes now return identical 18-model sets; after the source row is removed, the UI has 17 alternatives including PSA, UNI-ROYAL, YAGEO, TE, Vishay, RALEC, Susumu, EVER OHMS, and Sunway. The complete 56-test safety gate passes with protected data unchanged.

## 2026-08-26 - Valid copy events disappeared when the backend search snapshot lagged

- Symptom: a member copied `FRL2512FR039TS` from the result table and the copy request returned HTTP 201, but the backend still showed no copied model.
- Root cause: the copy ticket used the search row ID assigned by the Streamlit runtime (`#3068`), while another runtime's restored member-database snapshot still ended at `#3067`. The backend discarded copy events whose numeric search ID was absent from its currently visible search rows.
- Fix: give every new search a stable audit key, include the search/member/customer context in the HMAC-signed copy ticket, store that context with the D1 event, and join by the stable key after synchronization. Display all date-scoped copy events in an independent recent-copies table instead of filtering them through the local search snapshot; retain numeric-ID fallback for historical events.
- Follow-up finding: the formal runtime could also return a valid result table while failing to allocate any local search row, which previously suppressed the ticket entirely. Generate an independent signed audit key per submitted line as a fallback; permit `search_log_id=0` only when that key is present, so the recent-copies table remains complete without weakening ticket verification.
- Performance and privacy: the browser still observes only copy events originating from result-table brand/model cells, never reads the clipboard, never changes the copied text, and sends the audit request asynchronously. Backend reads are cached for five seconds and can be refreshed manually.
- Regression: stable-key joining is tested with intentionally mismatched numeric IDs, the additive local and D1 migrations are covered, and the complete 57-test release safety gate passes with protected runtime data unchanged.

## 2026-08-27 - Mixed-purpose resistor series labels blocked ordinary exact-part alternatives

- Symptom: searching PSA `FCF02FV-1501` displayed the source row but no other-brand alternative, while its electrical specification (0402, 1.5kΩ, ±1%, 1/16W) returned dozens of models across many brands.
- Root cause: the FCF series description covers both general thick-film and low-ohm current-sense products. Reverse lookup copied `电流检测/低阻值` onto every FCF model and treated those series-level labels as hard candidate constraints, even when the actual resistance was 1.5kΩ.
- Fix: apply source-metadata validation to every bare exact resistor lookup instead of adding another brand-only exception. When resistance is 10Ω or higher, discard contradictory `电流检测/低阻值` series labels from alternative filters while retaining the original source metadata for display. Credible properties such as automotive, anti-sulfur, surge, and high-voltage remain hard requirements; the existing FOJAN FRL electrical-core rule remains intact.
- Regression: `FCF02FV-1501` now returns at least 25 other-brand models across at least 12 brands, including FOJAN, Walsin, and YAGEO controls. FOJAN FRL exact/direct equivalence, YAGEO automotive routing, and cross-brand compliance constraints remain covered.

## 2026-08-27 - Search details stopped syncing after the audit database exceeded the snapshot limit

- Symptom: the backend search-detail table remained on August 25 with every copy count at zero, even after a new `FCF02FV-1501` result was copied. The independent D1 audit table proved that the copy arrived and retained the copied `华新科Walsin / WR04X1501FTL` context.
- Root cause: immutable per-result search auditing grew the member SQLite snapshot beyond the Worker endpoint's 2 MB decoded-payload limit. Whole-database uploads then failed with HTTP 413, leaving the backend on an older snapshot. Three result tables in the same page also emitted the same logical copy within milliseconds.
- Fix: gzip member SQLite snapshots before base64 transport, store the encoding and original size through additive D1 columns, and preserve backward-compatible reads of historical identity snapshots. Merge signed remote copy events into search details when their local search row is temporarily absent, and collapse identical events received within one second while retaining later intentional copies.
- Performance and safety: compression and upload remain in the existing coalesced background worker, so member searches and native copy operations do not wait. Decompression verifies the original SHA-256 and size and enforces a 64 MB ceiling. No protected runtime database is reset, replaced, or seeded.
- Regression: isolated tests cover compressed snapshot upload/restore, identity-snapshot compatibility, duplicate-event collapse, delayed intentional copies, remote-detail synthesis, additive Worker migration, and JavaScript/Python syntax. The complete 59-test release safety gate passes with protected runtime fingerprints unchanged. Formal D1 advanced from identity snapshot version 1229 to gzip version 1230: 1,998,848 original bytes became 313,560 base64 characters, and the backend counted the three raw FCF events as one logical copy.

## 2026-08-27 - Browser refresh discarded the active backend route

- Symptom: after entering the administrator backend through the formal page, pressing browser refresh returned to the ordinary matching page instead of keeping the backend open.
- Root cause: the navigation link changed only the embedded Streamlit iframe query to `admin=1`. The Cloudflare outer shell kept its original URL without a page-mode parameter, so a full browser refresh reconstructed the iframe as the default search route.
- Fix: the authenticated Streamlit bridge now reports its active `search / admin / member / bom` mode through the existing randomized, channel-checked route bridge. The outer shell replaces only those route parameters in its own URL, and forwards all three non-search modes when rebuilding the iframe. No member token is exposed in the visible URL.
- Regression: source checks cover route-mode messages, allowed-mode validation, outer history replacement, BOM forwarding, and the existing logout route clearing behavior. A formal browser replay confirmed the outer URL changed to `?admin=1`, remained `?admin=1` after a full reload, rebuilt an `admin=1` iframe, kept the backend visible, and exposed no member token in the address bar.

## 2026-08-27 - Copy audit appeared twice on the search-record page

- Symptom: the backend displayed a full-width `最近复制的品牌和型号` table even though the same copy count, brand/model, and time already belonged to rows in `查看搜索明细`.
- Root cause: the independent table was introduced as an emergency visibility fallback while the member snapshot lagged. After remote copy events began synthesizing missing search-detail rows, keeping both presentations duplicated the same information and added unnecessary vertical scrolling.
...
- Root cause: the outer shell persisted only the top-level `admin=1` route. The selected backend module lived only in Streamlit session state and was therefore unavailable when a full refresh rebuilt the iframe.
- Fix: assign every backend module a fixed non-sensitive slug, mirror the selected slug through the existing randomized and origin-checked route bridge, and restore it when the iframe is rebuilt. Invalid values are ignored. Leaving the backend, opening member/BOM pages, completing login, or logging out removes the module parameter.
- Privacy: the visible route stores only a fixed value such as `admin_module=search_logs`; it contains no member token, account, search text, customer, brand, or model data.
- Regression: focused tests cover the fixed allowlist, inner-to-outer synchronization, iframe reconstruction, cleanup, and backend default selection. A formal browser replay selected `搜索记录`, confirmed both shell and iframe carried `admin_module=search_logs`, performed a full F5, and remained on the same search-record module without exposing a member token.

## 2026-08-28 - Metric chip-package input left the size blank

- Symptom: `R-CHIP;47 ohm,1%,1/16W,1005` parsed the resistance, tolerance, and power but left the size blank, even though metric `1005` is the same package as imperial `0402`.
- Root cause: resistor and MLCC specification parsing accepted only the internal imperial package field. The legacy size normalizer also uses standalone `1005` as a padded historical imperial code, so applying that normalizer globally would have changed existing database meanings.
- Fix: add a passive-chip package conversion layer that recognizes unambiguous metric codes and explicit metric/imperial labels, converts metric input to the existing imperial matching key, and leaves the database schema unchanged. Resistor and MLCC specification/result tables now show separate `尺寸（公制）` and `尺寸（英制）` columns.
- Regression: the reported input resolves to metric `1005` / imperial `0402`, returns matching models, and shows both codes in the specification and result tables. Existing imperial `0402` input and an MLCC metric `1005` control resolve to the same pair. The complete 61-test release safety gate passes with protected runtime data unchanged.

## 2026-08-28 - Concatenated prefixed specifications were treated as one search

- Symptom: six pasted resistor specifications beginning with repeated `R-CHIP;` prefixes had no real newline between them, so the page reported `处理 1/1` and searched only the first specification instead of all six.
- Root cause: the search entry point split input only with Python `splitlines()`. The textarea's visual wrapping looked like multiple lines to the user but did not add newline characters to the submitted value.
- Fix: before applying line-count and length limits, split any physical line that starts with and repeats an allowlisted `R-CHIP;`, `C-CHIP;`, or `MLCC;` component prefix. Preserve ordinary newline behavior and leave text with content before the first prefix untouched to avoid splitting notes or prose accidentally.
- Regression: the exact reported string splits into six independent resistor queries with values 100kΩ, 1.5kΩ, 3.3kΩ, 39kΩ, 560Ω, and 75Ω; the first five retain metric `1005` / imperial `0402`. Mixed capacitor/resistor prefixes, normal single-line input, and non-leading prefix text are covered. The complete 63-test release safety gate passes with protected runtime databases unchanged.

## 2026-08-28 - Official PDC FBF order number could not be recognized

- Symptom: exact search `FBF05FTPR100` returned zero results even though it is a current PDC/PSA (信昌) FBF-series product.
- Root cause: the exact component sidecar had no row for this ordering-code spelling, and the generic resistor parser extracted only `R100` while missing the PDC-specific `05 / F / T / P` size, tolerance, packing, and power-type codes. With only one parameter, the search was rejected as unrecognized.
- Fix: implement the official FBF ordering grammar, validated size/packing combinations and resistance ranges, normal/power rating tables, type-specific dimensions, TCR bands, environmental metadata, and exact synthetic source rows backed by PDC's current FBF-REV.8.0.R10 datasheet. Bare FBF exact lookups search alternatives on the electrical core while retaining credible AEC-Q200 or anti-sulfur suffix requirements.
- Regression: `FBF05FTPR100` now displays a 信昌PDC source row for 0805 / 100mΩ / ±1% / 1/2W / ±100ppm/℃ / halogen-free and returns other-brand same-core candidates. The official `FBF06FT-3R00N` example remains valid as 1206 / 3Ω / ±1% / 1/4W.

## 2026-08-31 - Customer BOM identifiers caused slow, misleading batch matching

- Symptom: uploading the 155-row `上海昶音智能科技电阻-0817.xlsx` workbook stayed at 0% for more than ten seconds and advanced only six rows in about fifty seconds.
- Root cause: automatic column selection treated the customer-owned `昶音料号` values as manufacturer order numbers, selected the nearly empty `Category` column as the specification, and treated the numeric height column as quantity. Every row therefore performed an unsuccessful internal-ID lookup before falling back to the real specification; export assembly could repeat that same lookup after a specification match succeeded.
- Fix: prefer explicit manufacturer-code headers, include column completeness when selecting a specification, require quantity-header semantics instead of numeric content alone, and perform export-side exact-model lookup only when the winning match actually came from the model column.
- Regression: the reported workbook now maps `厂商代码 / 物料名称 / 中文名称 / 无数量列`; `昶音料号` is no longer searched as the primary model and `高度mm（max）` is no longer exported as quantity. The unchanged 155-row workbook produced its first result in 3.2 seconds and completed all rows in 108 seconds, with 148 recommended, 2 confirmation, and 5 genuine no-match rows. This is roughly twelve times the throughput shown in the original 6-row/53-second report.
- Deployment follow-up: the first formal restart repeatedly exceeded Streamlit Cloud's three-minute repository-clone window because the checkout contained both the bundled search database and 159 MB of redundant offline source catalogs. Keep those catalogs on the build workstation and in Git history, but exclude them from the live checkout; the deployment tree falls from 349.8 MB to 190.3 MB. A clean runtime snapshot with no source-catalog directories reconstructed the 578,699,264-byte search database from the release bundle and reported the component data ready.

## 2026-08-31 - Whole cost-list upload duplicated worksheet customer scope

- Symptom: the `整份清单` uploader required an administrator to choose between `新客户通用价` and `旧有客户专属价`, even though every worksheet already declares its own scope in B1. This duplicated the workbook rule and could assign a mixed general/customer workbook to the wrong whole-list scope.
...
## 2026-09-01 - BOM vendor/SMD descriptions hid the requested brand's detailed models

- Symptom: copied BOM descriptions such as `UNIOHM/SMD 0402 150Ω ±5% 1/16W LF`, `YAGEO/SMD 1206 800mΩ ±1% 1/4W LF`, and `WALSIN/SMD 0402 47Ω ±1% 1/16W LF` either failed to identify the vendor or removed that vendor from the returned model list. Embedded TA-I model `RLS10FTSR030` also inherited a generic 0805 power instead of its ordering-code power.
...
## 2026-09-17 - BOM matching and large-result review felt unresponsive

- Symptom: BOM matching spent several seconds per small group of rows, and a completed BOM with hundreds of rows remained sluggish while reviewing, filtering, or scrolling the result table.
- Root cause: every FOJAN model-rule attempt rebuilt the complete catalog regular-expression table, including repeated failed attempts for customer/internal item numbers. Within one candidate evaluation, the same exact-model database miss could also be requested repeatedly. The result page then generated one large HTML table for every matched row on each Streamlit rerun.
- Fix: compile the FOJAN catalog parser table once, cache normalized FOJAN model parses while returning defensive copies, and reuse exact-model lookup hits/misses only within the current candidate evaluation. Paginate the review table at 50 rows by default (50/100/200/all selectable) while keeping the downloaded workbook complete.
- Accuracy safety: matching order, component recognition, series constraints, resistance ranges, recommendation ranking, and pricing rules are unchanged. A representative 10-row mixed BOM produced the same key-result SHA-256 before and after the optimization; only execution and rendering work were reduced.
- Verification: the isolated mixed-BOM benchmark improved from 13.486 seconds to 9.140 seconds (about 32% faster) with identical status/model output. Rendering 416 review rows took 0.5802 seconds and generated 269,055 HTML characters, while the default 50-row page took 0.1117 seconds and 52,431 characters. Golden parser cases and explicit FRQ/FRT/FRV/FRR/FQH/FQP series/value controls pass.

## 2026-09-17 - Cost lookup could follow a stale series label or reject valid price dimensions

- Symptom: an exact FOJAN model could retain another series in a prepared row and therefore read the wrong series price. FRA array dimensions such as `064R 0603*4`, FRP 2010 rows whose commercial price table uses 1.5W, ordinary anti-sulfur FCS models, and FRM zero-ohm jumpers could also remain unpriced even though their own series page contained the applicable range. In a real batch run, a pandas row also triggered an ambiguous-truth exception in the new dimension selector.
- Root cause: price-series normalization inspected the row's series column before the exact order number. Price lookup then required the BOM/display power label to equal the cost sheet's commercial Type / Dimension text byte-for-byte. A shared `FCS` prefix was additionally treated as an alloy-only profile before checking whether the complete model actually followed the alloy-shunt format. Explicit-model pinning recognized catalog-series prefixes but not parsed alloy models, and the alloy value helper rejected the cost sheet's published `0mR` value.
- Fix: make a complete FOJAN model the highest series authority; constrain every lookup to that series; accept a cost-sheet dimension alias only when the same series and package has one unique published dimension; map FRA order-number array codes to their exact price dimensions; distinguish ordinary FCS models from the separate FCS alloy grammar; pin valid FOJAN alloy models exactly; admit only the FRM zero-ohm package/power combinations explicitly published in the cost sheet; and accept both dictionaries and pandas rows in batch pricing.
...
- Root cause: ordinary FOJAN order numbers were recognized only when their series already existed in the maintained component catalogue, so a future series could be imported into the price database without becoming an exact BOM candidate. Separately, an older built-in alloy catalogue profile vetoed 17 size/power/resistance combinations that the current uploaded cost workbook explicitly publishes across FWP, FCM, FPM, and FRM.
