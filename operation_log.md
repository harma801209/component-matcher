...
### 2026-08-24 [FOJAN current sense] Recognize 检流电阻 as an alloy-resistor specification

- Confirmed from FOJAN's official FRM product page that 0805, 0.5W, 5mΩ, and ±1% are supported; the source-scoped order-number rule resolves to `FRM0805FR005TM`.
- Fixed the missing simplified/traditional aliases `检流电阻` and `檢流電阻`, which previously left the screenshot input classified as an ordinary chip resistor and bypassed the FOJAN alloy generator.
- Added end-to-end regressions for `0805检流电阻(0.005Ω,1%,1/2W)` and its traditional-Chinese equivalent. Both now parse as alloy resistors and return the official-rule FOJAN model.
- Verification: Python compile check, focused regression, and the complete 54-test release safety gate passed; protected member, cost-list, and no-match database fingerprints remained unchanged.

### 2026-08-26 [FOJAN FRC] Explain non-standard resistance values without inventing models

- Reproduced all three reported searches against the current formal baseline. `240Ω ±5% 1206 1/4W` and `2MΩ ±5% 0402 1/16W` already return `FRC1206J241 TS` and `FRC0402J205 TS`; only `17KΩ ±1% 0603 1/10W` has no exact result.
- Verified in FOJAN's official FRC datasheet that the series is halogen-free and that ±1% resistance values use E24+E96. Since 17KΩ is in neither table, the matcher continues to reject the fabricated-looking `FRC0603F1702TS` instead of presenting it as an orderable part.
- Replaced the generic database warning for this case with a source-backed explanation that halogen-free is not the blocker. The page shows neighboring standard values 16.9KΩ and 17.4KΩ for engineering confirmation but never treats either as an exact substitute.
- Regression coverage locks the non-standard-value explanation and both valid reported searches. The complete 55-test release safety gate passed with the protected no-match database fingerprint unchanged; the protected member and cost databases were absent from this worktree and were not created or modified.

### 2026-08-26 [FOJAN FRL] Make exact-part alternatives match the equivalent electrical-spec search

- Reproduced `FRL2512FR039TS`: exact-part lookup found the FOJAN source row but returned no alternatives, while `2512 0.039R 1% 1W` returned 18 same-electrical-spec rows.
- The reverse lookup carried source-only `厚膜电阻 / 厚膜 / 200V / 低阻值` metadata into the candidate filter. The direct specification search used only package, resistance, tolerance, and power, so the two routes queried different scopes.
- Bare FOJAN FRL part-number lookup now preserves the complete original row for the source-information card but uses the same four electrical core fields as the equivalent direct specification when finding alternatives. Functional special requirements remain strict for non-FRL and explicitly specified searches.
- Regression coverage asserts identical model sets for both reported inputs, preserves the original FRL source card, and verifies the source row is removed only from the displayed alternative table. The complete 56-test release safety gate passed with protected runtime data unchanged.

### 2026-08-26 [Search audit] Trace copied result brands and models without slowing sales

- Added copy auditing only inside ordinary-search result tables. When a member copies text from a displayed brand or model cell with `Ctrl+C` or the browser copy command, the system records the associated search-log ID, member ID, source/matched group, displayed rank, brand, model, and server receipt time. It never reads or stores unrelated clipboard contents.
- Kept the sales interaction invisible and non-blocking: the native copy event is not canceled or changed, the page does not refresh, no notice is shown, and the formal shell sends the signed audit ticket with an asynchronous keepalive request after the copy action. Network or audit-service failure cannot block the copy operation.
- Added HMAC-signed, seven-day result-row tickets so the public browser can report only the exact brand/model rows issued by the server. The Cloudflare endpoint verifies the signature and bounded ticket fields before inserting a D1 audit event; administrative reads require the existing member-store secret.
- Expanded the backend search-record page with interval copy counts, per-search copy totals and latest copied brand/model, copied flags on the exact historical result rows, and a timestamped list of every copied brand/model for the selected search. A failed audit read is shown as unavailable rather than incorrectly reported as zero copies.
- Regression coverage verifies signed ticket contents, result-cell instrumentation, non-interference with native copy, asynchronous formal-shell delivery, and backend trace data. JavaScript and Python syntax checks pass, and the complete 57-test release safety gate passes with isolated databases and protected member, cost-list, and no-match runtime data unchanged.

### 2026-08-26 [Copy audit sync] Show copy records even when the search snapshot is one step behind

- Reproduced the reported live copy and confirmed that the browser POST succeeded with HTTP 201, but its signed ticket referenced search record `#3068` while the backend's latest restored search snapshot ended at `#3067`. The backend then filtered out the valid copy event because its numeric search record was not yet visible locally.
- Added a stable per-search audit key and carried it, together with the account, member display name, customer, original query, and search time, inside the signed result-row ticket. New copy events can now be joined to their search after delayed snapshot synchronization without relying on instance-local numeric IDs.
- Removed the backend's visible-search-ID filter and added an independent `最近复制的品牌和型号` table plus a manual refresh button. A newly received copy is therefore visible immediately even when the full search snapshot has not yet reached that backend runtime.
- Formal follow-up exposed a second race: when a runtime could not allocate a local search row at all, it rendered result cells without a copy ticket. Copy capture now creates an independent signed audit key and search/member/customer context for every submitted line, using the local search ID only when available. The D1 endpoint accepts a zero local ID only when that signed stable key is present.
- Kept native copy behavior unchanged and asynchronous; no clipboard read, page refresh, wait, or member-facing status was added. Existing copy rows remain readable through the numeric-ID compatibility path.
- Verification: Python and Worker syntax checks, three focused migration/copy regressions, and the complete 57-test release safety gate pass. Tests use isolated databases and protected runtime data is unchanged.
- Deployment: Cloudflare loaded the new Worker immediately, while Streamlit continued serving the previous Python result HTML after the Git push. The existing dependency-file marker was advanced without changing dependency versions to force a clean formal Streamlit rebuild.
- Formal verification: a live `2512 0.039R 1% 1W` search produced a 485-character signed ticket containing the stable audit key, account `terry46`, query text, and search ID `#3068`; copying `FRL2512FR039TS` returned HTTP 201. Remote D1 stored the same account/query/brand/model context, and the backend's recent-copies table visibly showed that row after refresh (six date-scoped copy events total).

### 2026-08-27 [Exact resistor lookup] Validate mixed-purpose series labels across brands

- Reproduced PSA `FCF02FV-1501`: the source row was 0402 / 1.5kΩ / ±1% / 1/16W, but its mixed-purpose FCF series description injected `电流检测/低阻值` as hard filters and reduced the alternative result to zero.
- Generalized source-metadata validation for every bare exact resistor lookup. At ordinary resistance values of 10Ω or higher, contradictory low-ohm/current-sense series labels are removed from alternative filters while retained on the source-information record. Credible automotive, anti-sulfur, surge, high-voltage, and similar properties remain strict.
- Added regressions for the reported PSA FCF model and special-series controls. The FCF lookup now returns at least 25 other-brand models across at least 12 brands; FOJAN FRL exact/direct equivalence and YAGEO automotive `AC1206FR-7W2R2L` behavior remain unchanged.
- Advanced the query cache and formal release stamps and refreshed the dependency-file rebuild marker. The complete 59-test release safety gate passed after the release-marker changes; protected member, cost-list, and no-match database fingerprints were unchanged.

### 2026-08-27 [Search audit sync] Compress snapshots and recover copy-backed details

- Confirmed the reported copy was not lost: remote D1 retained the signed `FCF02FV-1501` event and its copied `华新科Walsin / WR04X1501FTL` result, while the backend's member/search snapshot remained on August 25.
- Confirmed the snapshot upload failure in formal logs as HTTP 413. The member SQLite audit payload had outgrown the Worker's prior 2 MB decoded-payload ceiling.
- Added gzip transport for member snapshots with additive `payload_encoding` and `uncompressed_size` D1 columns, original-SQLite SHA-256/size validation, historical identity-payload compatibility, and a bounded decompression ceiling. Upload remains asynchronous and coalesced.
- Added copy-event deduplication for identical events received within one second and a remote-audit fallback row in `查看搜索明细` whenever the signed copy exists before its SQLite search row. Later intentional copies remain separate.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `118`, both public stamps to `2026-08-27T05:50:00+08:00`, the dependency rebuild marker, and the Cloudflare cache buster to `20260827-search-audit-gzip-1`.
- Verification: all 59 release-gate tests passed with unchanged protected database fingerprints. Deployed Cloudflare preview `https://d434248e.fruition-component.pages.dev`, pushed commit `07765864` to `origin/main`, and confirmed Streamlit logged `Updated app!` at 2026-08-27 05:54 local time.
- Formal live check: administrator login showed `结果复制次数 = 7`, correctly treating the three raw `FCF02FV-1501` events as one logical copy. The next member snapshot succeeded as version 1230 with `payload_encoding=gzip`; its 1,998,848-byte SQLite source is stored in 313,560 base64 characters, eliminating the prior HTTP 413 path.

### 2026-08-27 [Formal navigation] Keep backend open after browser refresh

- Reproduced the route design defect: `进入后台` navigated only the embedded Streamlit URL, while the outer `pages.dev` address retained no `admin=1`; F5 therefore rebuilt the default matching page.
- Added channel-validated active-page synchronization from Streamlit to the formal shell for `search / admin / member / bom`. The shell now stores the active mode with `history.replaceState`, forwards BOM as well as admin/member on reconstruction, and continues to strip the member token from the visible URL.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `119`, both public stamps to `2026-08-27T09:25:00+08:00`, the dependency rebuild marker, and the Cloudflare cache buster to `20260827-admin-route-refresh-1`.
- Verification: the complete 59-test safety gate passed with protected database fingerprints unchanged. Deployed Cloudflare preview `https://432df489.fruition-component.pages.dev`, pushed `ad7a985b` to `origin/main`, and confirmed Streamlit `Updated app!` at 2026-08-27 09:28 local time. A headless formal-browser login then entered the backend at outer URL `?admin=1`, performed a full reload, and remained in the backend with an `admin=1` iframe; the visible URL contained no member token.

### 2026-08-27 [Search audit UI] Merge recent copies into search details

- Removed the redundant page-level `最近复制的品牌和型号` table from the search-record backend.
- Moved `复制次数`, `已复制品牌/型号`, and `最近复制时间` directly behind `搜索内容` in `查看搜索明细`, so copied results are visible alongside their originating search without a second module. Distinct copied models are aggregated on the row; full event-level history remains in `追查单次搜索结果`.
- Kept remote copy-backed synthetic detail rows, including the no-local-summary fallback, so removing the duplicate table does not reintroduce the prior snapshot-lag visibility bug.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `120` and both public stamps plus the dependency rebuild marker to `2026-08-27T09:40:00+08:00`.
- Verification: the complete 59-test release safety gate passed with protected database fingerprints unchanged. Pushed `2d6bc421` to `origin/main`, confirmed Streamlit `Updated app!` at 2026-08-27 09:43 local time, and verified through the formal browser that the standalone recent-copy heading is absent while search details, the copy metric, single-search trace, and persistent backend route remain present.

### 2026-08-27 [Formal navigation] Keep the selected backend module after F5

- Extended the formal route state from the top-level backend flag to the selected internal backend module. Fixed slugs cover `无匹配回报`, `搜索记录`, `成本清单`, `客户资讯`, `会员审核`, `会员资料管理`, `数据质量`, and `运行状态`.
- Streamlit reports the selected slug through the existing channel-validated route bridge, and the outer shell forwards only allowlisted slugs when rebuilding the iframe. Leaving backend-related navigation clears the slug.
- The visible URL stores no member token or business input; a search-record route is represented only as `admin=1&admin_module=search_logs`.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `121`, the Cloudflare cache buster to `20260827-admin-module-refresh-1`, and finally both public stamps plus the dependency rebuild marker to `2026-08-27T10:12:00+08:00`.
- First formal replay finding: selecting `搜索记录` changed the rendered module but the route bridge still observed the previous `no_match` query value. Added one immediate Streamlit rerun after a changed module slug is written so the bridge publishes the confirmed new route rather than stale state.
- Verification: both complete safety-gate runs passed all 59 tests with unchanged protected database fingerprints. Deployed Cloudflare preview `https://a2f5b80f.fruition-component.pages.dev`, pushed application commits `420251bb` and `b7946f16` to `origin/main`, and confirmed Streamlit `Updated app!` at 2026-08-27 10:14 local time.
- Formal live replay: selected `搜索记录`, observed outer route `?admin=1&admin_module=search_logs`, performed a full browser reload, and confirmed the same route and module remained active. The rebuilt iframe also received the allowlisted slug, while the visible URL contained no member token.

### 2026-08-27 [Admin cost upload] Search and select existing customers

- Replaced the free-text customer field in `成本清单 / 整份清单 / 旧有客户专属价` with a searchable customer selector. Each maintained option includes the customer name, customer code, and customer group, so administrators can filter by any of those visible keywords before uploading.
- The selector combines active customer-information records with historical dedicated-price customers that have not yet been entered in customer information. Disabled maintained customers are excluded even if older dedicated prices still exist.
- Added an explicit `＋ 新增客户（清单中没有）` option. The manual customer-name field appears only after that option is chosen, and a successful dedicated-price upload makes the name available in later selections through the existing history path.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `122` and both public stamps plus the dependency rebuild marker to `2026-08-27T11:26:57+08:00`.
- Verification: the focused selector regression passed, then both complete release-gate runs passed all 60 tests with protected member, cost-list, and no-match database fingerprints unchanged. Pushed application commit `810df7d8` to `origin/main`; Streamlit logged `Updated app!` at 2026-08-27 11:30 local time.
- Formal live replay opened `?admin=1&admin_module=cost_price`, entered the whole-list upload tab, found two existing customer choices, filtered them by a typed keyword, selected one and verified its confirmation caption, then switched to the new-customer option and verified that the manual field appeared. No test file was uploaded and no formal cost data was changed.

### 2026-08-28 [Chip package size] Show metric and imperial codes separately

- Reproduced the reported `R-CHIP;47 ohm,1%,1/16W,1005` input: resistance, tolerance, and power parsed, but metric package `1005` was not recognized by the imperial-only matching field.
- Added an explicit passive-chip conversion table and contextual parser. Metric input is converted to the existing imperial matching key without changing the component database; explicit metric/imperial labels are supported and ambiguous codes keep the established imperial interpretation unless marked metric.
- Replaced the single size column with `尺寸（公制）` and `尺寸（英制）` for MLCC, chip/thick-film/thin-film resistors, and alloy resistors. Non-chip resistor and other component schemas retain their existing size display. Both new columns were widened after the first formal screenshot showed cramped headers.
- Regression coverage uses the exact reported input, an existing imperial `0402` control, and an MLCC metric `1005` control. The reported search resolves to `1005 / 0402`, returns 38 local candidates, and displays both codes in specification and result tables. The final release safety gate passed all 61 tests with protected runtime databases unchanged.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `124`, both public release stamps to `2026-08-28T13:41:52+08:00`, and pushed application commits `953dde85` and `637de345` to `origin/main`. Streamlit pulled the final change and logged `Updated app!` at 2026-08-28 13:46 local time.
- Formal live replay searched the exact reported line, visibly showed `尺寸（公制） = 1005` and `尺寸（英制） = 0402`, and returned cross-brand exact matches including FOJAN `FRC0402F47R0TS`, PSA FAF/FCF models, and Walsin `WR04X47R0FTL`. No formal component, member, customer, or cost record was modified.

### 2026-08-28 [Result tables] Auto-size columns from visible content

- Replaced fixed width classes in component specification tables with pixel widths calculated from each header and up to the first 100 visible rows. Short fields such as series, value, tolerance, and power now contract while longer descriptions retain enough room.
- Tightened the same shared estimator for matched-result tables, lowered the general minimum width to 64 pixels, kept practical minima for key short columns, and capped long columns at 320 pixels so unusually long text does not make the whole table unwieldy.
- Applied the content-aware widths to the specification tables, clickable matched-result tables, static result previews, and the dataframe fallback path. Horizontal scrolling remains available when the complete set of columns exceeds the viewport.
- Added regression coverage for compact metric/imperial size columns, expansion for genuinely long content, and sampling through row 100. The complete release safety gate passed all 62 tests with protected runtime database fingerprints unchanged.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `125` and both public release stamps plus the dependency rebuild marker to `2026-08-28T14:04:00+08:00`. Pushed application commit `71a1265f` to `origin/main`; Streamlit logged `Updated app!` at 2026-08-28 14:07 local time.
- Formal live replay searched `R-CHIP;100 kohm,5%,1/16W,1005`, completed in 2.8 seconds, retained the same cross-brand matching results, and visibly confirmed compact content-sized widths in both the specification and matched-result tables.

### 2026-08-28 [Search input] Split concatenated component specifications without line breaks

- Reproduced the pasted input in which six `R-CHIP;...` specifications were joined together with no real newline. The browser's visual wrapping was not a line separator, so the previous `splitlines()` path treated the whole paste as one search.
- Added a conservative input splitter for repeated `R-CHIP;`, `C-CHIP;`, and `MLCC;` prefixes. Ordinary text and ordinary newline-separated input are unchanged, while attached prefixed records are processed as separate searches with their own result and audit entries.
- Regression coverage uses the exact six-record reported string plus mixed capacitor/resistor and non-prefix controls. The complete release safety gate passed all 63 tests with protected runtime databases unchanged.
- Advanced `QUERY_RESULT_CACHE_VERSION` to `126` and all public release markers to `2026-08-28T14:23:26+08:00`. Pushed application commit `8b6739be` to `origin/main` and confirmed the formal service updated.
- Formal live replay processed `6/6`, rendered six independent specification sections, and returned results for every record; the last incomplete record remained a clearly labeled partial-parameter search instead of being silently discarded.

### 2026-08-28 [PDC FBF] Reverse-parse official current-sense resistor models

- Reproduced `FBF05FTPR100`: the local sidecar contained no exact source row and the generic resistor parser extracted only `R100`, so size, tolerance, power, source information, and other-brand alternatives were unavailable.
- Implemented the official PDC FBF ordering rules for supported sizes, tolerance codes, tape quantities, normal/power ratings, resistance encoding, dimensions, TCR bands, halogen-free status, and control suffixes. When the local library lacks the source row, an exact source-information row is generated from the official ordering code and links back to PDC's datasheet.
