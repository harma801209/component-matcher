...
const APP_CACHE_BUSTER = "20260813-security-boundary-1";
...
async function handleMemberStoreSnapshot(request, env) {
  ...
  await env.MEMBER_DB.prepare(
    `INSERT OR IGNORE INTO member_auth_snapshot_history (version, sha256, payload_base64, updated_at)
     SELECT version, sha256, payload_base64, updated_at FROM member_auth_snapshots WHERE id = 1`,
  ).run();
  ...
  if (request.method === "GET") {
    ...
    const row = requestedVersion > 0
      ? await env.MEMBER_DB.prepare(
        "SELECT version, sha256, payload_base64, updated_at FROM member_auth_snapshot_history WHERE version = ? LIMIT 1",
      ).bind(requestedVersion).first()
      : await env.MEMBER_DB.prepare(
        "SELECT version, sha256, payload_base64, updated_at FROM member_auth_snapshots WHERE id = 1 LIMIT 1",
      ).first();
    if (requestedVersion > 0 && !row) {
      return memberStoreJson({ error: "snapshot_not_found", version: requestedVersion }, 404);
    }
    return memberStoreJson(row || { version: 0, sha256: "", payload_base64: "", updated_at: "" });
  }
  ...
  if (payloadBytes.byteLength > 2_000_000) {
    return memberStoreJson({ error: "payload_too_large" }, 413);
  }
  ...
  if (currentVersion === 0) {
    writeResult = await env.MEMBER_DB.prepare(
      "INSERT OR IGNORE INTO member_auth_snapshots (id, version, sha256, payload_base64, updated_at) VALUES (1, ?, ?, ?, ?)",
    ).bind(nextVersion, suppliedSha, payloadBase64, updatedAt).run();
  } else {
    writeResult = await env.MEMBER_DB.prepare(
      "UPDATE member_auth_snapshots SET version = ?, sha256 = ?, payload_base64 = ?, updated_at = ? WHERE id = 1 AND version = ?",
    ).bind(nextVersion, suppliedSha, payloadBase64, updatedAt, currentVersion).run();
  }
      ...
      (() => {
        ...
        window.addEventListener("message", (event) => {
          ...
          if (payload.source === "fruition-route") {
            ...
            if (payload.action !== "clear-page-modes") return;
